...
function App() {
  ...
  useEffect(() => {
    const fetchAllQuestions = async (lessonId) => {
      try {
        ...
        for (let num = 0; num <= 9; num++) {
          promises.push(fetch(`http://127.0.0.1:8080/question?lessonId=${lessonId}&questionNumber=${num}`));
        }
        ...
        await Promise.all(promises)
          .then(results => {
            ...
            results.forEach(response => f.push(response));
          })
          .catch(error => {
    ...
    if (lesson === 0) {
      fetchAllQuestions().then(f => {setFlashcards(f)})
    }
  }, [lesson])
  
  const whatToRender = () => {
    if (lesson === 0 && flashcards) {
      return <Flashcard lessonSummary={lessonSummary} flashcards={flashcards} setLesson={setLesson}></Flashcard>
